...
export function VolunteerForm(){
 ...
 return <form className="volunteer-form" onSubmit={(e)=>{e.preventDefault();setSent(true)}}><label>Full name<input required type="text" placeholder="Your name"/></label><label>Email address<input required type="email" placeholder="you@example.ca"/></label><label>Home region<select required defaultValue=""><option value="" disabled>Select your region</option><option>Ambrosia</option><option>Chasmia</option><option>Myrati</option><option>Oplana</option><option>Reno</option><option>Montiablo</option><option>Another region</option></select></label><button className="button button-white" type="submit">Count me in <ArrowIcon/></button><small>No data leaves this local preview.</small></form>
}
